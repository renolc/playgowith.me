...
Meteor.methods({
  ...
  play: function (playerId, position) {
    ...
    enemyClusters.forEach(function (cluster) {
      if (cluster.liberties.length <= 1) {
        cluster.cells.forEach(function (cell) {
          ...
          console.log(Cluster.getNextToCell(game._id, playerId, cell).fetch());
    ...
    if (friendlyClusters.length > 0 && friendlyClustersWithNoLibertiesCount === friendlyClusters.length) {
      throw new Meteor.Error('invalid move');
    }

    // if no friently clusters and this space has no liberties, invalid move
    liberties = getLibertiesByPosition(game.board, position);
    if (friendlyClusters.length === 0 && liberties.length === 0) {
      throw new Meteor.Error('invalid move');
    }
    ...
    var cluster = Cluster.create(playerId, position, liberties);
