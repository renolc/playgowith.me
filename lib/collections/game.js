...
Meteor.methods({
  ...
  play: function (playerId, position) {
    ...
    enemyClusters.forEach(function (cluster) {
      if (cluster.liberties.length <= 1) {
        ...
        game.lastCapturedClusters.push(cluster._id);
      }
    }, this);
    ...
    // mark captured clusters as deadGam
    ...
    Game.update(game._id, {
      $set: {
        ...
        lastCapturedClusters: game.lastCapturedClusters
      }
    });
  },
  ...
  approve: function(playerId) {
    ...
    // all marked clusters should be made dead
    Cluster.getMarked(game._id).forEach(function(cluster) {
      cluster.markDead();
      cluster.cells.forEach(function(cell) {
        game.board[cell[0]][cell[1]] = null;
      }, this);
    ...
    for(var x = 0; x < game.board.length; x++) {
      for (var y = 0; y < game.board.length; y++) {
        if (visited.indexOf(getAreaKey(x, y)) !== -1)
          continue;
...
function getArea(board, x, y, results) {
  ...
  var key = getAreaKey(x, y);
  if (results.area.indexOf(key) !== -1)
    return results;

  if (board[x][y] === null) {
    results.area.push(key);
  } else {
